...
FROM employee
JOIN branch 
ON employee.emp_id = branch.mgr_id;--join employee table and branch table together into 1 table, and join them together on a specific column
...
FROM employee
LEFT JOIN branch 
ON employee.emp_id = branch.mgr_id; --all employees got included
...
WHERE client.branch_id IN (
    ...
    FROM branch
    WHERE branch.mgr_id = 102
    LIMIT 1 -- just in case he is the manager at multiple branches
);
...
WHERE emp_id = 102; -- delete michel scott from employee
-- ON DELETE SET NULL EXAMPLE
SELECT * from branch; -- manager id is now set to null, becuase we added that previously

SELECT * from employee; --super id now set to null, because we added on delete set to null to employee table

-- ON DELETE CASCADE EXAMPLE
DELETE FROM branch
WHERE branch_id = 2;

SELECT * FROM branch_supplie; -- any row with brand ID is deleted now ADD

INSERT INTO employee
VALUES (109, 'Oscar', 'Meyer', '2022-08-02', 'M', 350000, NULL, NULL);
